    ...
    return {
        ...
        add: function (request) {
                ...
                thread.Send({run: "Project.get", param: {id: projectId}})
                    .then(function(data){
                        ...
                        listView = new L5.UI.ListView({
                            header: "<span>name</span><span>default value</span><span>Option</span>",
                            container: "#api-list-view",
                            rowTpl: "<span><input type='text' name='name' value='${name}'></span>" +
                                    "<span><input type='text' name='value' value='${value}'></span>" +
                                    "<span><input type='checkbox' name='option' value='${option}'></span>"
                        });
                    });
            }
            // 表单提交事件处理
            view.Listen("#api-form", "submit",  function(e){
                ...
                data['params'] = listView.GetResult();
            ...
            if (id > 0) {
                thread.Send({run: "Api.get", param: {id: id}})
                    .then(function (data) {
                        ...
                        oldOptions = data.params.slice();
                        L5.ViewManager.Assign('info', data);
                        return thread.Send({run: "Project.get", param: {id: projectId}});
                    ...
                    .then(function (data) {
                        ...
                        listView = new L5.UI.ListView({
                            ...
                            rowTpl: "<span><input type='text' name='name' value='${name}'></span>" +
                            "<span><input type='text' name='value' value='${value}'></span>" +
                            "<span><input type='checkbox' name='option' checked='${option}'></span>"
                        });
                    });
            }
        ...
        remove: function (request) {
            var id = request['id']|0,
                projectId;
            if (id > 0) {
                thread.Send({run: "Api.delete", param: id})
                    .then(function (data) {
                        L5.UI.Dialog.Show("删除成功");
                        L5.Controller.GoPrev(); // 返回上一页
                    });
            }
        }
    };
