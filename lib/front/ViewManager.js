...
(function ($) {
    ...
    View.prototype._delegateHandle = function (evt) {
        ...
        var handles = listeners[evt.type].slice(),
            i, len,
            key,
            tar = evt.target;
    ...
    View.prototype.Listen = function (selector, eventType, handle) {
        if (!this._listeners[evtType]) {
            ...
            this.context.addEventListener(evtType, delegateHandle, false);
        }
        var handles = this._listeners[evtType].slice(), wasBind = false;
        ...
        !wasBind && this._listeners[evtType].push({"k": selector, "v": handle});
        return this;
    };
